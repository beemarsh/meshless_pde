classdef Square
   ...
   % numPoints: number of points in each direction.  This now refers to
   % the number of *interior* points, not including the boundaries.
   xmin = obj.Domain(1);
   xmax = obj.Domain(2);
   ymin = obj.Domain(3);
   ymax = obj.Domain(4);
   % Add a small offset to avoid placing points exactly on the boundaries.
   % This ensures strict interior points.
   offset_x = (xmax - xmin) / (numPoints + 1);
   offset_y = (ymax - ymin) / (numPoints + 1);
   % Create a uniform grid over the *interior* of the square.
   % Use linspace with adjusted start and end points.
   x_domain = linspace(xmin + offset_x, xmax - offset_x, numPoints);
   y_domain = linspace(ymin + offset_y, ymax - offset_y, numPoints);
