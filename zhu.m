...
%analytical eigenvalue parameters of the PDE
% (Note: the order of the pairs may not consistent with the order obtained from the numerical method)

% The analytical solution to the above PDE IS:
% u(x,y)=sin(m*pi*x)*sin(l*pi*y)   -> This is the eigen function,
% where m = 1, 2, 3, ....
% and where l = 1, 2, 3, ....

% Also the associated eigenvalues are:
% lambda=(m^2+l^2)*pi^2
...
for nn=nodes
    ...
    scatter(coor(:,1),coor(:,2), 15, 'filled', 'g');
    ...
    for c=sp
        ...
        display(size(approximate_eigenvalues));
        ...
        if ii==1&jj==1
            ...
            for j=1:9
                ...
                axis equal;
                axis([0,1,0,1]);
                title("Exact Eigenmodes for Shape Parameter : " + c);
            ...
            for j=1:9
                ...
                axis equal;
                axis([0,1,0,1]);
                title("Numerical Eigenmodes for Shape Parameter : " + c);
            ...
            figure
            for j=1:9
                coor(int_ind,4)=abs(abs(exact_eigenmode(int_ind,j))-abs(normf(j)*alpha(1:ni,j)));
                Z=reshape(coor(:,4),size(X));
                subplot(3,3,j)
                %contourf(X,Y,Z,20);
                surf(X,Y,Z);
                ...
                axis equal;
                axis([0,1,0,1]);
                title("Absolute Error for Shape Parameter : " + c);
