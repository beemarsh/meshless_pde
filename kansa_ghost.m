%% Kansa Method with Ghost Points
...
shapes = linspace(0.00001 , 5, 500);
% shapes= [3.28];

radii = linspace(0.8,1.5,20); % Radius of the circle

m = [1 2 1 2 3 1 3 2 4 ];
l = [1 1 2 2 1 3 2 3 1 ];

aph = 2.6; % Parameter of the ghost circle (alpha)

nn = 20 % It forms a (nn+1)^2 sized grid

print_eigenvalues = false; % Prints eigenvalues in the console if set to true

%% Store the results

result_eigenvalues = zeros(length(m), length(shapes), length(radii)); % Approximate eigenvalues

error_eigenvalues = zeros(length(m), length(shapes), length(radii)); % Absolute error in eigenvalues

max_err_eigenmodes = zeros(length(m), length(shapes), length(radii)); % Maximum error in eigenmodes

relative_err_eigenmodes = zeros(length(m), length(shapes), length(radii)); % Relative error in eigenmodes

rms_err_eigenmode = zeros(length(m), length(shapes), length(radii)); % RMS error in eigenmodes
...
for R=radii
    ...
    [x_ghost, y_ghost] = fabric_pattern(num_total_pts, aph, 0.5, 0.5, R);
    ...
    for shape = shapes
        ...
        DM = D(coordinates(:,1:2), ghost_pts(:,1:2)); % Find the distance matrix
...
% [best_shapes_eigvalue, best_errors_eigvalue] = best(error_eigenvalues, "Shapes", shapes, "Radius", radii, true);

% [best_shapes_eigmodes, best_errors_eigmodes] = best(max_err_eigenmodes, "Shapes", shapes, "radius", radii, true);


%% Plot the Errors

% plot_eigenvalue_errors(radii,shapes,error_eigenvalues,l,m); %Plot the eigenvalues errors

% plot_eigenmode_abs_error(radii,shapes,max_err_eigenmodes,l,m); % Plot the absolute error of eigenmodes

% plot_eigenmode_rel_error(radii,shapes,relative_err_eigenmodes,l,m); % Plot the relative error of eigenmodes

% plot_eigenmode_rms_error(radii,shapes,rms_err_eigenmode,l,m); % Plot the RMS error of eigenmodes
