function plot_eigenvalue_errors(nodes,shapes,error_eigenvalues,l,m)
    ...
    for j = 1:length(shapes)
            ...
            for i = 1:length(nodes)
                    ...
                    scatter3((nodes(i)+1)^2*ones(1,length(m)), 1:length(m), log10(squeeze(error_eigenvalues(:,j,i))), 'filled', color(i));
                    hold on;
                    % Use plot3 to connect the points with lines
                    plot3((nodes(i)+1)^2*ones(1,length(m)), 1:length(m), log10(squeeze(error_eigenvalues(:,j,i))), '-o', 'LineWidth', 1.5);
                    hold on;
            end
            % Labeling the axes
