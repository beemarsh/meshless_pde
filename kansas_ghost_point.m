...
% Unit square domain
square_domain = Square([-1, 1, -1, 1]);
square_domain = square_domain.generateGrid(40);
square_domain = square_domain.generateGhostPoints(5, 0.05);
%square_domain.scatterPlot("Square Domain", true, true);  %Plot the domain
...
ghost_points = square_domain.GhostPoints;
all_points = [collocation_points; boundary_points; ghost_points];

num_collocation_points = length(collocation_points(:,1));
num_boundary_points = length(boundary_points(:,1));
num_ghost_points = length(ghost_points(:,1));
num_total_points = num_collocation_points + num_boundary_points + num_ghost_points;

%The Grid is Stored in The Object. Now, extract the Grid.
X = square_domain.GhostGrid.X;
Y = square_domain.GhostGrid.Y;
...
% Lets find the nearest boundary point.
idx = knnsearch(boundary_points, ghost_points); % Finds the nearest boundary point from the ghost point.

nearest_boundary_point = boundary_points(idx, :); % holding the (x,y) of the nearest boundary point
...
%for shape = shapes
    ...
    %For interior points
    DM_i = D(collocation_points, all_points); % Distance matrix for interior points

    DM_b = D(boundary_points, all_points); % Distance matrix for boundary points

    DM_g = D(nearest_boundary_point, all_points); % Distance matrix for Ghost points using nearest boundarties

    DM = [DM_i; DM_b; DM_g]; % Combine to get a N*N distance matrix
