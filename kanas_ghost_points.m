...
for nn=nodes
        ...
        for shape = shapes
                ...
                % plot_exact_eigenmodes(length(m), exact_eigenmode, X, Y, shape);

                % Plot the numerical eigenmodes
                % plot_numeric_eigenmodes(length(m), coordinates, X, Y, interior_idx, num_interior_pts, normf, alpha, shape);
